PROGRAM p113
!-------------------------------------------------------------------------
! Program 11.3 Forced vibration analysis of an elastic solid in plane
!              strain using rectangular 8-node quadrilaterals. Lumped or
!              consistent mass. Mesh numbered in x- or y-direction.
!              Implicit time integration using the "newmark beta" method.
!-------------------------------------------------------------------------
 USE main
! USE geom
 IMPLICIT NONE
 INTEGER,PARAMETER::iwp=SELECTED_REAL_KIND(15)
 INTEGER::i,j,iel,k,loaded_nodes,ndim=2,ndof=16,nels,neq,nip=9,nlen,nn,   &
   nod=8,nodof=2,npri,nprops=3,np_types,nr,nres,nst=3,nstep,nxe,nye
 REAL(iwp)::area,c1,c2,c3,c4,det,dtim,one=1.0_iwp,beta,gamma,time,       &
   zero=0.0_iwp,c5,c6,two=2.0_iwp
   LOGICAL::consistent=.true.
 CHARACTER(LEN=15)::argv,element='quadrilateral'
!-----------------------dynamic arrays------------------------------------
 INTEGER,ALLOCATABLE::etype(:),g(:),g_g(:,:),g_num(:,:),kdiag(:),nf(:,:), &
   node(:),num(:)   
 REAL(iwp),ALLOCATABLE::bee(:,:),coord(:,:),dee(:,:),der(:,:),deriv(:,:), &
   d1x0(:),d1x1(:),d2x0(:),d2x1(:),ecm(:,:),fun(:),f1(:),g_coord(:,:),    &
   jac(:,:),km(:,:),kv(:),loads(:),mm(:,:),mv(:),points(:,:),prop(:,:),   &
   val(:,:),weights(:),x0(:),x1(:),x3(:),x2(:),x_coords(:),y_coords(:)
!-----------------------input and initialisation--------------------------
 CALL getname(argv,nlen)
 OPEN(10,FILE=argv(1:nlen)//'.dat')
 OPEN(11,FILE=argv(1:nlen)//'.res')
 READ(10,*)nxe,nye,np_types
 CALL mesh_size(element,nod,nels,nn,nxe,nye)
 ALLOCATE(nf(nodof,nn),points(nip,ndim),g(ndof),g_coord(ndim,nn),         &
   dee(nst,nst),coord(nod,ndim),jac(ndim,ndim),weights(nip),der(ndim,nod),&
   deriv(ndim,nod),bee(nst,ndof),km(ndof,ndof),num(nod),g_num(nod,nels),  &
   g_g(ndof,nels),mm(ndof,ndof),ecm(ndof,ndof),fun(nod),etype(nels),      &
   prop(nprops,np_types),y_coords(nye+1),x_coords(nxe+1))
 READ(10,*)prop
 etype=1
 IF(np_types>1)READ(10,*)etype
 READ(10,*)y_coords,dtim,nstep,beta,gamma,npri
 x_coords = meshx(nxe,0.40_iwp)
 nf=1
 READ(10,*)nr,(k,nf(:,k),i=1,nr)
 CALL formnf(nf)
 neq=MAXVAL(nf)
 ALLOCATE(x0(0:neq),d1x0(0:neq),x1(0:neq),d2x0(0:neq),loads(0:neq),       &
   d1x1(0:neq),d2x1(0:neq),kdiag(neq),x2(0:neq),x3(0:neq))
 CALL sample(element,points,weights)
 kdiag=0
!-----------------------loop the elements to find global array sizes------
 elements_1: DO iel=1,nels
   CALL geom_rect(element,iel,x_coords,y_coords,coord,num,'y')
   CALL num_to_g(num,nf,g)
   g_num(:,iel)=num
   g_coord(:,num)=TRANSPOSE(coord)
   g_g(:,iel)=g
   CALL fkdiag(kdiag,g)
 END DO elements_1
 CALL mesh(g_coord,g_num,argv,nlen,12)
 DO i=2,neq
   kdiag(i)=kdiag(i)+kdiag(i-1)
 END DO
 ALLOCATE(kv(kdiag(neq)),mv(kdiag(neq)),f1(kdiag(neq)))
! WRITE(11,'(2(A,I5))')                                                    &
!   " There are",neq," equations and the skyline storage is",kdiag(neq)
 kv=zero
 mv=zero
!-----------------------global stiffness and mass matrix assembly---------
 elements_2: DO iel=1,nels
   CALL deemat(dee,prop(1,etype(iel)),prop(2,etype(iel)))
   num=g_num(:,iel)
   coord=TRANSPOSE(g_coord(:,num))
   g=g_g(:,iel)
   km=zero
   mm=zero
   area=zero
   gauss_pts_1: DO i=1,nip
     CALL shape_der(der,points,i)
     CALL shape_fun(fun,points,i)
     jac=MATMUL(der,coord)
     det=determinant(jac)
     CALL invert(jac)
     deriv=MATMUL(jac,der)
     CALL beemat(bee,deriv)
     km=km+MATMUL(MATMUL(TRANSPOSE(bee),dee),bee)*det*weights(i)
     area=area+det*weights(i) 
     IF(consistent)THEN
       CALL ecmat(ecm,fun,ndof,nodof)
       mm=mm+ecm*det*weights(i)*prop(3,etype(iel))
     END IF
   END DO gauss_pts_1
   IF(.NOT.consistent)CALL elmat(area,prop(3,etype(iel)),mm)
   CALL fsparv(kv,km,g,kdiag)
   CALL fsparv(mv,mm,g,kdiag)
 END DO elements_2 
! --------------------auto-compute loaded_nodes and node, as well as val.
! READ(10,*)loaded_nodes
loaded_nodes = 400
 ALLOCATE(node(loaded_nodes),val(loaded_nodes,ndim))
! Call loaded_nodes_computation(loaded_nodes,node,val,nres,g_num)
do i = 1,loaded_nodes
val(i,:) = (/0.0_iwp,1.0_iwp/)
node(i) = 8+(i-1)*5
end do
nres = MAXVAL(node)
! READ(10,*)(node(i),val(i,:),i=1,loaded_nodes) 
 !--------------------------------end ----------------------------------
 !-----------------------initial conditions and factorise equations--------
 x0=zero
 d1x0=zero
 d2x0=zero
 !-------------------------------Newmark beta algorithm-------------------
 c1 = gamma/beta/dtim
 c2 = one/beta/dtim/dtim 
 f1 = kv+c2*mv
 c3 = one/beta/dtim 
 c4 = one/two/beta-one 
 c5 = one-gamma/beta 
 c6 = one-gamma/two/beta 
 CALL sparin(f1,kdiag)
 time=zero
!-----------------------time stepping loop--------------------------------
! WRITE(11,'(/A,I5)')" Result at node",nres
! WRITE(11,'(A)')"    time        load        x-disp      y-disp"
 WRITE(11,'(12E12.4)')time,load(time),x0(nf(:,nres)),d1x0(nf(:,nres)),d2x0(nf(:,nres))
 timesteps: DO j=1,nstep
   time=time+dtim
   loads=zero
   x1 = c2*x0+c3*d1x0+c4*d2x0 
   x2 = c1*x0-c5*d1x0-c6*d2x0 
   DO i=1,loaded_nodes
     loads(nf(:,node(i)))= val(i,:)*(load(time))
     if (i.eq.1 .or. i.eq.loaded_nodes) then
       loads(nf(:,node(i))) = 0.5_iwp*loads(nf(:,node(i)))
     end if
   END DO
   CALL linmul_sky(mv,x1,d1x1,kdiag)
   x3 = loads+d1x1
!   loads=c2*x0
!   CALL linmul_sky(kv,loads,x1,kdiag)
!   x1=d1x1
   CALL spabac(f1,x3,kdiag)
   d1x1 = c1*x3-x2
   d2x1 = c2*x3-x1
   x0=x3
   d1x0=d1x1
   d2x0=d2x1
   IF(j/npri*npri==j)WRITE(11,'(12E12.4)')time,load(time),x0(nf(:,nres)),d1x0(nf(:,nres)),d2x0(nf(:,nres))
 END DO timesteps
STOP
CONTAINS
FUNCTION load(t) RESULT(load_result)
!-----------------------Load-time function--------------------------------
 IMPLICIT NONE     
 INTEGER,PARAMETER::iwp=SELECTED_REAL_KIND(15)
 REAL(iwp),INTENT(IN)::t
 REAL(iwp)::load_result
! load_result=COS(0.3_iwp*t)
if (t.le.0.02_iwp) then
  load_result = -25*t
elseif (t.le.0.04_iwp) then
  load_result = -25*(0.04_iwp-t)
else
  load_result = 0.0_iwp
end if
RETURN
END FUNCTION load

function meshx(nxe,xlen) result(x_coords)
  !----------------------automesh in x-direction -------------------------
  implicit none 
  integer,parameter::iwp = selected_real_kind(15)
  integer,intent(in)::nxe
  real(iwp),intent(in)::xlen
  real(iwp)::x_coords(nxe+1)
  integer::i
  do i=0,nxe
  x_coords(i+1) = xlen*i/nxe
  end do
  end function meshx 

!subroutine loaded_nodes_computation(loaded_nodes,node,val,nres,g_num)
!  implicit none
!  integer,parameter::iwp = selected_real_kind(15)
!  integer,intent(in)::g_num(:,:)
!  integer,intent(out)::loaded_nodes,node(:),nres
!  real(iwp),intent(out)::val(:,:)
!  integer::i
!  !
!  loaded_nodes = UBOUND(g_num,2)
!  do i = 1,loaded_nodes
!  node(i) = maxval(g_num(:,i))
!  val(i,:) = (/0.0_iwp,1.0_iwp/)
!  end do
!  nres = maxval(node)
!  end subroutine loaded_nodes_computation
END PROGRAM p113
